    ...
    <form method="POST" action="{{ route('reminders.update', $reminder) }}" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3"
                class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 
                       dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $reminder->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select id="status" name="status"
                class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 
                       dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500">
                <option value="Pendiente" {{ $reminder->status == 'Pendiente' ? 'selected' : '' }}>Pendiente</option>
                <option value="Completado" {{ $reminder->status == 'Completado' ? 'selected' : '' }}>Completado</option>
                <option value="Vencido" {{ $reminder->status == 'Vencido' ? 'selected' : '' }}>Vencido</option>
            </select>
        </div>
